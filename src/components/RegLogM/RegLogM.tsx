...
const RegLogM = forwardRef<HTMLDivElement, RegLogMProps>(
  ({ action, close = null, updateUser, goto = "" }, ref) => {
    ...
    const [logErr, setLogErr] = useState<[boolean, boolean]>([false, false]);
    ...
    const sendToServ = async (e: React.FormEvent<HTMLFormElement>, act: number) => {
      e.preventDefault();
      const userData = {
        firstname: nameInput,
    ...
    return (
      ...
      >
        <div id="switchRegLogM">
          ...
          <Switch checked={switcher} onChange={handleChange} />
          <h2 style={{ color: theme.palette.text.primary }}>Регистрация</h2>
        </div>
        <div id="regLogM">
          <motion.div
            initial={action ? { display: "none" } : { display: "flex" }}
            animate={controlsLog}
            id="LoginM"
            className="regLogDataWrapper"
          >
            ...
            <Button form="lInputsM" type="submit" className="regLogBnt" variant="contained">
              Войти
            </Button>
          </motion.div>
          <motion.div
          ...
          >
            ...
            <Button form="rInputsM" type="submit" className="regLogBnt" variant="contained">
              Войти
            </Button>
          </motion.div>
        </div>
