...
interface BasketProductProps
{
  ...
  uniqueKey: number, 
  check: boolean, 
  deleteFunc: (uniqueKey:number) => void, 
  checkFunc: (uniqueKey:number) => void,
  delAllFlag: boolean,
}

const BasketProduct:React.FC<BasketProductProps> = ({prdtTitle, prdtDescription, prdtDiameter, prdtWeight, prdtCategory="", prdtPrice, imgLink="", id, uniqueKey, check, deleteFunc=null, checkFunc, delAllFlag }) => {
  const { theme } = useTheme();
  const [checked, setChecked] = useState(check);
  const [loading, setLoading] = useState(true);
  let navigate = useNavigate();
  
  const returnWeightCategory = (weight:number) => {
  ...
  const startAnimation = async () => {
    ...
    deleteFunc?deleteFunc(uniqueKey):console.log('ban on deletion')
  };
